...
function renderSketchToDataUrl(sketch?: SketchState | null) {
  ...
  const drawChamber = () => {
    switch (sketch.chamberShape) {
      case 'Square':
        // Further reduced to match live preview proportions
        ctx.strokeRect(cx - 90, cy - 90, 180, 180)
        break
      case 'Rectangle':
        // Further reduced (taller than wide) to match live preview
        ctx.strokeRect(cx - 60, cy - 110, 120, 220)
        break
      case 'Hexagon': {
        // Further reduced to match live preview proportions
        ctx.beginPath()
      ...
      default:
        // Further reduced circle to match live preview
        ctx.beginPath()
...
async function addChartsPage(doc: jsPDF, records: DetailedManholeRecord[]) {
  ...
  for (const [loc, list] of groups) {
    ...
    const points = list
      ...
      .filter((p): p is { id: string; invert: number } => p.invert !== null)
